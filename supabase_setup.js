...
// Bu betik, Supabase veritabanında gerekli tabloları ve politikaları oluşturmak için kullanılır.
// Supabase SQL Editor'da veya Supabase CLI ile çalıştırılabilir.

// Supabase bağlantısı
const SUPABASE_URL = 'https://owwucdnfvzrwrqxseuyg.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im93d3VjZG5mdnpyd3JxeHNldXlnIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1Mjk1NzQ4MSwiZXhwIjoyMDY4NTMzNDgxfQ.31D4tpg5GwLGSZNbgiU4d7WEQBszkN6L19HkN3PY7Yo'; // Service Role Key kullanılmalıdır

// Supabase istemcisini oluştur
const { createClient } = require('@supabase/supabase-js');
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

// SQL sorgularını çalıştırmak için fonksiyon
async function runSQL(sql) {
  try {
    const { data, error } = await supabase.rpc('pgexec', { sql });
    if (error) throw error;
    console.log('SQL başarıyla çalıştırıldı:', data);
    return data;
  } catch (error) {
...
async function setupDatabase() {
  ...
  // database_schema.sql dosyasının içeriğini buraya ekleyin
  const sql = `
  -- Profiller Tablosu
  ...
  try {
    ...
    console.error('Veritabanı şeması oluşturulurken hata oluştu:', error);
  }
}
